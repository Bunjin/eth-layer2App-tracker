...
class Layer2App {
  ...
  constructor (opts = {}) {
    const { address, name, nodeUrl, balance, owner, provider, blockTracker, networkId } = opts
  ...
  async updateLayer2State(){
    const state = await this.script.updateLayer2State((state)=>{
      this.layer2State = state
      //console.log("layer 2 state ", state)
      return this.layer2State
    })
  }
